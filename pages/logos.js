...
export default function LogoManager() {
  ...
  const updateLogo = async (vendorName, updates) => {
    ...
    const finalData = { name: vendorName, logo_url: existing.logo_url, is_hidden: existing.is_hidden, ...updates };
    ...
    <div className="min-h-screen bg-zinc-50 font-sans text-zinc-900 p-6 md:p-12">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-12">
           <Link href="/" className="flex items-center gap-2 text-zinc-400 hover:text-black transition-all font-bold uppercase text-xs tracking-widest italic"><ArrowLeft size={16} /> Back</Link>
        ...
        <div className="grid grid-cols-1 gap-3">
          {data.vendors.map(vendor => {
            ...
            return (
              <div key={vendor} className={`bg-white p-4 rounded-3xl border transition-all flex flex-col md:flex-row items-center gap-6 shadow-sm ${saved?.is_hidden ? 'opacity-40 grayscale' : 'border-zinc-200'}`}>
                ...
                <div className="flex-grow w-full">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-black uppercase text-[10px] tracking-widest text-zinc-400">{vendor}</h3>
                  ...
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                      placeholder="Shopify Logo URL..." 
                      className="flex-grow p-4 bg-zinc-50 border border-zinc-100 rounded-xl text-[11px] font-mono outline-none focus:border-black transition-all"
                    ...
                    <button 
                      ...
                      className={`p-4 rounded-xl transition-all ${saved?.is_hidden ? 'bg-red-500 text-white shadow-lg' : 'bg-zinc-100 text-zinc-400 hover:bg-zinc-200'}`}
                      title={saved?.is_hidden ? "Unignore Vendor" : "Ignore Vendor"}
                    >
