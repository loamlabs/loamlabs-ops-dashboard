...
export default function OpsDashboard() {
  ...
  // PERSISTENCE: Check for saved session on load
  useEffect(() => {
    const savedPass = localStorage.getItem('loam_ops_auth');
    if (savedPass) {
      setPassword(savedPass);
      fetchRules(savedPass);
    }
  }, []);
  ...
  const fetchRules = async (passToUse) => {
    ...
    try {
      ...
      if (res.ok) { 
        ...
        if (passToUse) alert("Session Expired");
      }
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50 font-sans text-zinc-900">
      ...
      <aside className="w-64 bg-black text-zinc-400 p-6 hidden md:flex flex-col border-r border-zinc-800">
        <div className="mb-12">
          <img src="/logo.png" alt="LoamLabs" className="h-10 mb-2 object-contain invert opacity-90" onError={(e) => { e.target.style.display = 'none'; }} />
      ...
      <main className="flex-grow p-6 md:p-12 overflow-auto">
                ...
                <tr key={rule.id} className={`${rule.needs_review ? 'bg-red-50' : 'hover:bg-zinc-50'} transition-colors`}>
                  <td className="p-6">
                    <div className="flex items-center gap-2">
                       ...
                       {rule.review_reason && <div className="group relative cursor-help text-red-500"><Info size={14} /><div className="hidden group-hover:block absolute left-full ml-2 w-48 p-3 bg-black text-white text-[10px] rounded-xl shadow-2xl z-10 leading-relaxed font-normal italic">{rule.review_reason}</div></div>}
                    </div>
                    <div className="text-[10px] text-zinc-400 font-mono mt-1 truncate max-w-sm">{rule.vendor_url}</div>
                  </td>
                  <td className="p-6 text-center">
                    {rule.needs_review ? <span className="bg-red-600 text-white text-[9px] font-black px-3 py-1 rounded-full animate-pulse uppercase">Review Required</span> : rule.last_availability ? <span className="bg-green-100 text-green-700 text-[9px] font-black px-3 py-1 rounded-full uppercase italic">Active</span> : <span className="bg-zinc-200 text-zinc-600 text-[9px] font-black px-3 py-1 rounded-full uppercase">Out of Stock</span>}
                  </td>
                  <td className="p-6 font-mono font-bold text-lg text-zinc-700">${(rule.last_price / 100).toFixed(2)}</td>
                  <td className="p-6 flex justify-end text-right">
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200 text-sm">
