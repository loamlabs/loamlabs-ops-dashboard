...
export default async function handler(req, res) {
  ...
  try {
    // Change .insert to .upsert to prevent duplicate variant errors
    const { data, error } = await supabase.from('watcher_rules').upsert(rules, { onConflict: 'shopify_variant_id' });
